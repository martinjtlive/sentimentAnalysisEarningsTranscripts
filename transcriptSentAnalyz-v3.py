...
def main():
    ...
    with st.expander('More details ...'):
        ...
        st.text('''
    Specify Company name. Load the transcript files. Click 'Process'.
    For each file, text is chunked and preprocessed. 
    ...
    df_docresult = pd.DataFrame(columns=['company','publicationDate', 'avg.Positive','avg.Negative'
    ...
    # open file to append the company name into
    companyFile = open(save_df_location +'companyList.txt', 'a+')




    # layout for entry of files and display of metrics for a company
    entrycol, compselect = st.columns([.55,.45])
    ...
    with entrycol:
            ...
            # button2 = st.button('Save Dataframe to file')
            ...
            if button1:
                for file in pdf_files:
                    ...
                    # add row to df
                    new_row = [str(company), publishDate, avg_positive, avg_negative, avg_neutral, doc_sent]
                    
                    df_docresult.loc[len(df_docresult)] = new_row
                    writeMessages(f'Processed file: {file.name}')

                # sort df results by publication Date
                df_docresult.sort_values(by=['publicationDate'], inplace=True)
                    
                writeMessages('All files processed!!!!')    
                df_docresult.to_csv(save_df_location + company + '-sentMetrics.csv', index=False)

                # check if company is in companyList.txt. If not, append to file
                companyFile.seek(0)
    ...
    with compselect:
        ...
        # get company list from companyList file. Make into a pd sequence. Have a dropdown
        companyFile.seek(0)
        # read all content of file and then split by new line
        companyList = companyFile.read().splitlines()
      

        companyListSequence = pd.Series(companyList)
        
        # Add 'None' to top of the pandas series
        companyListSequenceWNone = pd.concat([pd.Series(['None']), companyListSequence]) # companyListSequenceWNone - companyListSequence with 'None'

        selectedCompany = st.selectbox( 'See sentiment based on earnings calls for which company', companyListSequenceWNone)

       
    # display dataframe and chart as 2 tabs below
    df, metricschart = st.columns([.5,.5])

    if selectedCompany in companyList:

        # load df from location
        full_file_location = save_df_location + selectedCompany + '-sentMetrics.csv'
        metricsdf = pd.read_csv(full_file_location)

        # sort by publication Date  (this is duplicative as it occurs also after file processing. Nevertheless keeping here)
        ...
        with metricschart:
            # plot line chart
            st.line_chart(metricsdf, x='publicationDate', y=['avg.Positive', 'avg.Negative', 'avg.Neutral'])
